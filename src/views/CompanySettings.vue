  ...
  methods: {
    async fetchUsers() {
      try {
        ...
        const response = await axios.get("https://signaturegenerator.samueldev.com/api/v1/users", {
    ...
    async updateUser() {
      try {
        ...
        await axios.put(
          `https://signaturegenerator.samueldev.com/api/v1/update-company-info?userId=${this.selectedUser.id}`,
          updateData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    async updatePhoneNumber() {
      try {
        const token = localStorage.getItem("token");
        await axios.put(
          `https://signaturegenerator.samueldev.com/api/v1/update-phone?userId=${this.selectedUser.id}`,
